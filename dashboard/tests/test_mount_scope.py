...
def test_no_dstack_home_mount_in_c22():
    ...
    dstack_mounts = [
        m for m in mounts
        if m.get("Source", "").startswith(dstack_dir)
    ]
...
def test_whitelisted_mounts_only():
    ...
    for m in mounts:
        ...
        # Also allow dstack single-file mount in C2.1a
        if not ok and ".dstack" in source:
            ok = source.endswith("config.yml") or "/projects/" in source
