...
from gpupoor.remote_env import find_dstack_bin, load_remote_settings
...
def fix_wsl_clock(*, max_skew_seconds: int | None = None) -> None:
    limit = max_skew_seconds or int(os.environ.get("MAX_CLOCK_SKEW_SECONDS", "5"))
...
def run_preflight(*, remote: bool = False) -> None:
    if os.environ.get("SKIP_PREFLIGHT") == "1":
        print(
            "WARNING: SKIP_PREFLIGHT=1 — skipping all preflight checks (operator-only; CI must not set)",
            file=sys.stderr,
        )
        return
    ...
    repo = repo_root()
    _run_local_preflight(reporter, repo)
    if reporter.failed:
    ...
    _run_remote_preflight(reporter, repo)
    if reporter.failed:
...
def _run_local_preflight(reporter: PreflightReporter, repo: Path) -> None:
    ...
    if shutil.which("powershell.exe"):
        ...
        if win_ts:
            ...
            if skew >= 5:
                reporter.fail(f"clock skew {skew}s between WSL2 and Windows (must be < 5s) — run: ./run.sh fix-clock")
        else:
...
def _run_remote_preflight(reporter: PreflightReporter, repo: Path) -> None:
    ...
    remote_env_path = repo / ".env.remote"
