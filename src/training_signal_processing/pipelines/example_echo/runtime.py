...
from .exporter import EchoExporter
from .models import RecipeConfig
from .resume import EchoResumeLedger


class EchoPipelineRuntimeAdapter(PipelineRuntimeAdapter):
    def __init__(
        self,
        *,
        config: RecipeConfig,
    ...
    def build_exporter(self) -> Exporter:
        return EchoExporter(self.object_store)

    def build_resume_ledger(self) -> ResumeLedger:
        return EchoResumeLedger(config=self.config, object_store=self.object_store)
