...
class YoutubeAsrResumeLedger(ResumeLedger):
    ...
    def load_completed_item_keys(self, run_id: str) -> set[str]:
        ...
        for key in self.object_store.list_keys(prefix):
            ...
            for row in self.object_store.read_jsonl(key):
                result = YoutubeTranscriptResult.from_dict(row)
                if result.status == "success":
                    completed_keys.add(result.source_media_r2_key)
    ...
    def commit_batch(
        ...
        *,
        ...
        results = [YoutubeTranscriptResult.from_dict(row) for row in rows]
