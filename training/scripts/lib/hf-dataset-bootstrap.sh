...
hf_dataset_bootstrap() {
    ...
    if [ -f "$dataset_file" ]; then
        ...
        if [ "$actual_bytes" -lt "$dataset_min_bytes" ]; then
            echo "$log_prefix Found truncated dataset at $dataset_file (${actual_bytes} bytes) — deleting for re-download" >&2
            rm -f "$dataset_file"
        fi
    fi
    ...
    if [ ! -f "$dataset_file" ]; then
        ...
        if ! python3 - "$data_dir" "$hf_dataset_repo" "$hf_dataset_filename" "$log_prefix" <<'PYEOF'
import os
import sys

data_dir, repo_id, filename, log_prefix = sys.argv[1:5]
...
PYEOF
    ...
    if [ "$actual_bytes" -lt "$dataset_min_bytes" ]; then
        echo "$log_prefix ERROR: Dataset file is only ${actual_bytes} bytes (expected >= ${dataset_min_bytes})" >&2
        echo "$log_prefix File may be corrupt or truncated — deleting for next run" >&2
        rm -f "$dataset_file"
        return 1
    fi

    echo "$log_prefix Dataset OK: $(ls -lh "$dataset_file" | awk '{print $5}') @ $dataset_file"
}
