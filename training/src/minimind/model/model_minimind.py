    ...
    def forward(self, x, position_embeddings, past_key_value=None, use_cache=False, attention_mask=None):
        ...
        if self.flash and (seq_len > 1) and (not self.is_causal or past_key_value is None):
            output = F.scaled_dot_product_attention(
                ...
                is_causal=sdpa_mask is None and self.is_causal,
            )
        else:
...
class MiniMindModel(nn.Module):
    def __init__(self, config: MiniMindConfig):
        ...
        self.layers = nn.ModuleList(
            [MiniMindBlock(layer_index, config) for layer_index in range(self.num_hidden_layers)]
        )
        ...
        for _ in range(max_new_tokens):
            ...
            if eos_token_id is not None:
                next_token = torch.where(
                    finished.unsqueeze(-1), next_token.new_full((next_token.shape[0], 1), eos_token_id), next_token
                )
