...
def test_top_level_run_wrapper_delegates_to_cli() -> None:
    ...
    assert 'config="${1:-examples/tiny_local.toml}"' in script
    assert 'python3 -m gpupoor.cli train "$config" "$@"' in script
    assert 'python3 -m gpupoor.cli launch dstack "$config" "$@"' in script
    assert "python3 -m gpupoor.cli dstack teardown" not in script
