    ...
    for mount in TMPFS_MOUNTS:
        try:
            result = subprocess.run(
                ["docker", "exec", CONTAINER_NAME, "df", "-B1", mount], capture_output=True, text=True, timeout=5
            )
...
def _check_enospc_in_logs():
    ...
    try:
        result = subprocess.run(
            ["docker", "logs", "--tail", "1000", CONTAINER_NAME], capture_output=True, text=True, timeout=10
        )
...
def test_tmpfs_pressure_under_50_percent():
    ...
    for mount, (used, size) in usage.items():
        if size > 0:
            ratio = used / size
            assert ratio < 0.5, f"tmpfs {mount} is {ratio:.1%} full ({used}/{size} bytes) — over 50% threshold"
...
def test_tmpfs_mounts_have_expected_sizes():
    ...
    assert "128m" in tmp_opts.lower() or "134217728" in tmp_opts, f"/tmp tmpfs size not 128m: {tmp_opts}"
