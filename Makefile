...
PY_DIRS := $(wildcard src/gpupoor tests training/src/minimind training/tests)
REQUIRED_TEST_DIRS := tests training/tests
FAST_MARKERS := not docker and not remote and not slow
ARTIFACT_DIR := .artifacts
JUNIT_XML := $(ARTIFACT_DIR)/junit.xml
COVERAGE_XML := $(ARTIFACT_DIR)/coverage.xml
REQUIRED_TEST_COMMAND = PYTHONHASHSEED=0 TZ=UTC $(PYTEST) $(REQUIRED_TEST_DIRS) -m "$(FAST_MARKERS)" --strict-config --strict-markers --junitxml=$(JUNIT_XML) --cov=src/gpupoor --cov=training/src/minimind --cov-report=xml:$(COVERAGE_XML) --cov-report=term-missing:skip-covered
...
test-live:
	$(PYTEST) tests training/tests \
		-m "docker or remote or slow" \
		--strict-config \
		--strict-markers \
		-ra
